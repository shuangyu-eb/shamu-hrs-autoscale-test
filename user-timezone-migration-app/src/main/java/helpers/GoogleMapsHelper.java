...
public class GoogleMapsHelper {
  ...
  public String findTimezone(final TimezoneAddress timezoneAddress) {
    final String zipCodeTimezone = findTimezoneByZipCode(timezoneAddress.getPostalCode());
    if (zipCodeTimezone != null) {
      return zipCodeTimezone;
    }

    if (timezoneAddress.getCity() != null || timezoneAddress.getStateProvinceCode() != null) {
      final String cityStateTimezone =
          findTimezoneByCityState(
              timezoneAddress.getCity(), timezoneAddress.getStateProvinceCode());
      if (cityStateTimezone != null) {
        return cityStateTimezone;
      }
      return findTimezoneByState(timezoneAddress.getStateProvinceCode());
    }
  ...
  private Optional<LatLng> findLocationByState(final String state) throws Exception {
  ...
  private Optional<LatLng> findLocationByCityState(final String city, final String state)
    ...
    results =
        GeocodingApi.geocode(geoCodingApiContext, "")
            .components(
                new ComponentFilter("administrative_area", state),
  ...
  private Optional<LatLng> findLocationByPostalCode(final String postalCode) throws Exception {
    final GeoApiContext geoCodingApiContext =
        new GeoApiContext.Builder().apiKey(googleGeoCodingApiKey).build();
    final GeocodingResult[] results;
    results =
  ...
  private String findTimezoneByLocation(final LatLng latLng) throws Exception {
    final GeoApiContext mapApiContext =
        new GeoApiContext.Builder().apiKey(googleGeoCodingApiKey).build();
  ...
  public String findUserTimeZone(final String companyId, final String userId) throws Exception {
    final String officeAddressTimezoneId =
        TenantManager.getUserOfficeAddressTimezoneId(companyId, userId);
    if (officeAddressTimezoneId != null) {
      return officeAddressTimezoneId;
    }
    final TimezoneAddress homeAddress = TenantManager.getHomeAddress(companyId, userId);
    if (homeAddress != null) {
      final String timezone = findTimezone(homeAddress);
      if (timezone != null) {
        return TenantManager.findTimezoneIdByName(companyId, timezone);
      }
    }
    final String officeAddressIdWithMostEmployees =
        TenantManager.findOfficeAddressIdWithMostEmployees(companyId);
    if (officeAddressIdWithMostEmployees != null) {
      return officeAddressIdWithMostEmployees;
    }
    return TenantManager.findTimezoneIdByName(companyId, "America/Chicago");
  }
}
